      ...
      <div className="bg-slate-900 border border-slate-700 w-full max-w-4xl rounded-3xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 border-t border-slate-800 flex justify-between items-center bg-slate-950">
          <div className="flex items-center gap-3">
            <div className="text-sm font-bold text-slate-300 uppercase">Play Sound on Arrival</div>
            <button
              onClick={() => onUpdateProfile({ ...profile, playEndSound: !profile.playEndSound })}
              className={`w-12 h-6 rounded-full transition-colors relative ${profile.playEndSound !== false ? 'bg-amber-500' : 'bg-slate-700'}`}
            >
              <div className={`absolute top-1 w-4 h-4 rounded-full bg-white transition-transform ${profile.playEndSound !== false ? 'left-7' : 'left-1'}`} />
            </button>
          </div>
        </div>
      </div>
